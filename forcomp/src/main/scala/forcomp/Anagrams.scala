...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase()
      ...
      .groupBy(c => c)
      .map {
        case (ch, chList) => (ch, chList.length)
      }.toList
      .sortWith(_._1 < _._1)
  }
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    dictionary.map(word => (wordOccurrences(word), word)).groupBy(_._1).map {
      case (occurrences, wordList) => (occurrences, wordList.map(_._2))
    }
  }

  /** Returns all the anagrams of a given word. */
  def wordAnagrams(word: Word): List[Word] = {
    dictionaryByOccurrences.find(_._1 == wordOccurrences(word)).get._2
  }
  ...
  def findNextWords(occurenceList: List[Occurrences], filterList: List[Word]): Option[List[Word]] = {
    if (occurenceList.nonEmpty) {
      filter(dictionaryByOccurrences.get(occurenceList.head), filterList) match {
        case None => findNextWords(occurenceList.tail, filterList)
  ...
  def filter(target: Option[List[Word]], filterList: List[Word]): Option[List[Word]] = {
    (target, filterList) match {
      case (None, _) => None
      case (Some(w), x :: _) => {
        val filtered = w.filter(f => !filterList.contains(f))
