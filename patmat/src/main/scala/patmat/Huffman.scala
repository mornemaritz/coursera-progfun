...
object Huffman {
   ...
   */
    def until(singleton: List[CodeTree] => Boolean, combine: List[CodeTree] => List[CodeTree])(src: List[CodeTree]): CodeTree = {
          singleton(src) match {
            case true => src.head
            case _ => until(singleton, combine)(combine(src))
          }
    }
   ...
   */
    ...
    def decodeChar(subTree: CodeTree,remBits: List[Bit]): List[Char] = {
      (subTree, remBits.length) match {
        case (Leaf(c, _),0) => List(c)
        case (_,0) => List.empty
        case (Leaf(c,_),_) => List(c) ::: decode(tree, remBits)
        case (Fork(l,r,_,_),_) if remBits.head == 0 => decodeChar(l, remBits.tail)
        case (Fork(l,r,_,_),_) if remBits.head == 1 => decodeChar(r, remBits.tail)
      }
    }
   ...
   */
    ...
    def traverse(remText: List[Char], encodedText: List[Bit]): List[Bit] = {
      remText.length match {
        case r if r == 0 => encodedText
        case _ => traverse(remText.tail,encodedText ::: encodeChar(tree, remText.head, List.empty))
      }
    }

    traverse(text, List.empty)
  }
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = List.empty // ???
